...
class Circle {
    ...
    public int getQueue() {
        ...
        //判断队列是否空‘
        if (isEmpty()) {
            throw new RuntimeException("队列空不能获取数据");
        }

        int tem = arr[front];//front 就指向当前变量 取值后位移
        front = (front + 1) % maxSize;
        return tem;
    }
    ...
    public static void main(String[] args) {
        Circle arrayQueue = new Circle(3);
        char key = ' ';
        Scanner scanner = new Scanner(System.in);
        boolean loop = true;
        while (loop) {
